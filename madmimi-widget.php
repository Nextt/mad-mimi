	    ...
	    function widget($args, $instance) {
	 	...
	 	function mimi_signup_form($args, $shortcode_id = false, $show_title = false) {
			...
			if(isset($_POST['success']) && isset($_POST['mimi_form_id']) && $_POST['mimi_form_id'] == $this->number) { // The form has been submitted
				...
				$value = isset($_POST['signup']['email']) ? esc_attr($_POST['signup']['email']) : '';
				$out .=	"		<label for='signup_email{$this->number}'>Receba as notícias da IASD semanalmente no seu e-mail:<span class='required' title='This field is required'>*</span></label><br /><input id='signup_email{$this->number}' name='signup[email]' type='text' value='$value' /><br />";
